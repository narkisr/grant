  ...
  (is (= (:sudoers (process (sudoers (slurp "test/resources/single-line-no-passwd"))))
         '{:user-spec ({:user-list {:user "re-ops"}}
                       ...
                       {:cmnd-spec-list ({:cmnd-spec ({:runas-spec {:runas-list {:runas-member {:alias-name "ALL"}}}}
                                                      {:tag-spec "NOPASSWD:"}
                                                      {:cmnd {:commandname ({:file "/usr/bin/apt"}
                                                                            {:arg "update"})}})}
                                         {:cmnd-spec {:cmnd {:commandname ({:file "/usr/bin/apt"}
                                                                           {:arg "upgrade"}
                                                                           {:flag "-y"})}}}
                                         {:cmnd-spec {:cmnd {:commandname {:file "/usr/bin/purge-kernels"}}}}
                                         {:cmnd-spec {:cmnd {:commandname {:file "/usr/bin/apt-cleanup"}}}}
                                         {:cmnd-spec {:cmnd {:commandname ({:file "/usr/bin/apt-get"}
                                                                           {:arg "install"}
                                                                           {:wildcard "*"}
                                                                           {:flag "-y"})}}}
                                         {:cmnd-spec {:cmnd {:commandname ({:file "/usr/sbin/ufw"}
                                                                           {:arg "status"})}}}
                                         {:cmnd-spec {:cmnd {:commandname ({:file "/usr/bin/nmap"}
                                                                           {:wildcard "*"})}}}
                                         {:cmnd-spec {:cmnd {:commandname ({:file "/usr/bin/netstat"}
                                                                           {:flag "-tnpa"})}}})})})))
